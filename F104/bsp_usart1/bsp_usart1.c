...
void USART_NVIC_Config() //配置中断控制器
{
...
void Usart_SendHalfWord(uint16_t ch)
{
    ...
    temp_l = ch & 0XFF;
    /* 发送高八位 */
    USART_SendData(DEBUG_USARTx, temp_h);
    while (USART_GetFlagStatus(DEBUG_USARTx, USART_FLAG_TXE) == RESET);
    /* 发送低八位 */
    USART_SendData(DEBUG_USARTx, temp_l);
    while (USART_GetFlagStatus(DEBUG_USARTx, USART_FLAG_TXE) == RESET);
}

void Usart_SendString_Length(uint8_t *str, uint32_t num)
{
    unsigned int k = 0;
    do
    {
